...
class PPOAgent:
    ...
    def select_action_batch(self, obs: dict):
        ...
        """
        pov = torch.as_tensor(obs["pov"], dtype=torch.float32, device=self.device) / 255.0
    ...
    def update(self, rollout: dict, next_obs: dict) -> dict:
        ...
        advantages, returns = self.compute_gae(
            ...
            rollout["dones"],   next_vals,
            self.gamma,         self.gae_lambda,
        )

        # Convert rollout to tensors — flatten (T[,N], …) → (T[*N], …)
        povs    = torch.as_tensor(_flat("povs",      np.uint8,   n_feat=3),
                                  dtype=torch.float32, device=self.device) / 255.0
        invs    = torch.as_tensor(_flat("invs",      np.float32, n_feat=1),
                                  dtype=torch.float32, device=self.device)
        actions = torch.as_tensor(_flat("actions",   np.int64),
                                  dtype=torch.long,    device=self.device)
        old_lps = torch.as_tensor(_flat("log_probs", np.float32),
                                  dtype=torch.float32, device=self.device)
