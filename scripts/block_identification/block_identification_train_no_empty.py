...
WANDER_STEPS      = 70     # steps to wander before attempting to mine
STOP_STEPS        = 20     # no-op steps to let the player stop moving
PICKUP_STEPS      = 20     # steps to walk forward and pick up the dropped block
SCAN_SPEED        = 25.0
CONFIDENCE_THRESH = 0.70   # skip mining if KNN is at least this confident
...
def knn_max_confidence(model, pov: np.ndarray) -> float:
    """
    Flatten the POV frame and return the highest class probability
    from the KNN model. Returns 0.0 if no model is loaded.
    """
    if model is None:
        return 0.0
    flat = flatten_pov(pov).reshape(1, -1)      # (1, H*W*3)
    probs = model.predict_proba(flat)          # (1, n_classes)
...
def random_wander_action(env):
    ...
    a["camera"]  = np.array([
        0, # np.random.uniform(-SCAN_SPEED/2, SCAN_SPEED/2), 
...
def walk_forward_action(env):
    a = noop(env)
    a["forward"] = 1
    a["jump"] = int(np.random.random() > 0.7)
    return a
...
class BlockDataCollector:
    ...
    State machine:
      ...
      STOP    → hold still for STOP_STEPS; then check KNN confidence:
                  < 70%  → transition to MINE (uncertain block, worth labelling)
                  ≥ 70%  → skip back to WANDER (already know this block)
      ...
      LABEL   → diff inventory to label the observation, save to DB
    """
    ...
    def _stop_step_with_knn(self, obs):
        self._stop_ctr += 1
        if self._stop_ctr >= STOP_STEPS:
            pov        = obs["pov"].copy()
            confidence = knn_max_confidence(self.knn_model, pov)
            print(f"[KNN] Confidence: {confidence:.2%}")

            if confidence > CONFIDENCE_THRESH:
                # Model already knows this block well — skip it
                print(f"[KNN] Confident (>{CONFIDENCE_THRESH:.0%}), skipping → WANDER")
                self._wander_ctr = 0
                self._state      = self.WANDER
            else:
                # Uncertain — mine it to generate a new labelled sample
                print(f"[KNN] Uncertain (<={CONFIDENCE_THRESH:.0%}), mining → MINE")
                self._pov_before = pov
