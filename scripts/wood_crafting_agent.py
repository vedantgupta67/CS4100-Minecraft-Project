...
def evaluate(
    ...
    while ep < n_episodes:
        ...
        try:
            ...
            if logs > 0:
                steps_per_log.append(steps / logs)

            logger.info("Episode %2d: reward=%.2f  steps=%d logs=%d success=%s", ep + 1, total_rew, steps,int(logs),success)
            ep += 1
    ...
    logger.info("Avg time to 1st log : %.1f steps", avg_time_to_log)
    logger.info("Avg steps per log   : %.1f", avg_steps_per_log)